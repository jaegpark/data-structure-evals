...
from transformers import pipeline, AutoTokenizer, DistilBertForTokenClassification, BertForTokenClassification, BertTokenizer
...
"""
class StagedKG(Memory):

    def __init__(self, text, NER, RE):
        self.text = text
        self.ner_method = NER
        self.re_method = RE
        self.ner_entities = []

        # TWO STAGE APPRAOCH. first, find entities through NER
        
        # second, find relations between entities

        # third, construct graph
    ...
    def bert_ner(self):
        # assume bert_model and bert_tokenizer are the pretrained model and tokenizer for BERT NER
        ...
        self.ner_entities = [(token, label) for token, label in zip(tokens, predicted_labels) if "B-" in label or "I-" in label]
...
kg = StagedKG(text=test_text, NER="CoreNLP", RE="BERTRE")
